...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="pickout/list.do" method="post" name="Form" id="Form">
						...
						<div class="widget-toolbox padding-8 clearfix">
							<div class="btn-group">
								<button class="btn btn-primary btn-mini" onclick="search();" title="检索" data-rel="tooltip">
									...
									<c:if test="${QX.add == 1 }">
										<a class="btn btn-mini btn-info"  onclick="manualPick(1);"><span class="icon-ok"></span>拣货</a>
									</c:if>

								<c:if test="${QX.add == 1 }">
									<a class="btn btn-mini btn-success"  onclick="manualPickCommon(1);"><span class="icon-ok"></span>通用拣货</a>
								</c:if>

								<c:if test="${QX.add == 1 }">
									<a class="btn btn-mini btn-info"  onclick="manualPickSprea(1);"><span class="icon-ok"></span>SPREA拣货</a>
								</c:if>


								<c:if test="${QX.add == 1 }">
									<a class="btn btn-mini btn-warning" onclick="cancelPick(4);"><span class="icon-undo"></span>取消拣货</a>
								</c:if>
      ...
      //LOADED_TYPE_S  ASSIGNED_STATE_S DEPOT_STATE_S  PICK_STATE_S  OUTSTOCK_TYPE_S OUTSTOCK_WAY_S
		...
		function manualPick(opttype) {
			...
			if (str == '') {
				...
				diag.Drag = true;
				diag.Title = title1;
				diag.URL = '<%=basePath%>pickout/goPickManual.do?ASSIGN_TYPE=' + opttype +"&STOCKMGROUT_IDS="+str;
				diag.Width = 1100;
				diag.Height = 800;
		...
		function cancelPick(opttype) {
			...
			if (str == '') {
				...
				diag.Drag = true;
				diag.Title = title1;
				diag.URL = '<%=basePath%>pickout/goPickCancel.do?ASSIGN_TYPE=' + opttype +"&STOCKMGROUT_IDS="+str;
				diag.Width = 1200;
				diag.Height = 800;
				diag.CancelEvent = function () { //关闭事件
					search();
		...
		function manualPickSprea(opttype) {
			var title1 = "拣货";
			top.jzts();
			var diag = new top.Dialog();
			diag.Drag = true;
			diag.Title = title1;
			diag.URL = '<%=basePath%>pickout/goPickManualSprea.do?ASSIGN_TYPE=' + opttype;
			diag.Width = 1000;
			diag.Height = 800;
			diag.CancelEvent = function () { //关闭事件
				search();
		...
		function manualPickCommon(opttype) {
			var title1 = "通用拣货";
			top.jzts();
			var diag = new top.Dialog();
			diag.Drag = true;
			diag.Title = title1;
			diag.URL = '<%=basePath%>pickout/goPickManualCommon.do?ASSIGN_TYPE=' + opttype;
			diag.Width = 1000;
			diag.Height = 800;
			diag.CancelEvent = function () { //关闭事件
				search();
		...
		function look(Id){
			...
			diag.Drag=true;
			diag.Title ="查看拣货明细";
			diag.URL = '<%=basePath%>pickout/goLook.do?STOCKMGROUT_ID='+Id;
			diag.Width = 1200;
			diag.Height = 800;
			diag.CancelEvent = function(){ //关闭事件
