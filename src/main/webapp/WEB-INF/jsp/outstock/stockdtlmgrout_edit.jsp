...
<script type="text/javascript">
	...
	function getLocator(){
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrout/getLocatorList.do',
	...
	function hadProd(PRODUCT_ID,STOCKMGROUT_ID) {
		var result = false;
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrout/hadProd.do',
			data: {PRODUCT_ID:PRODUCT_ID,STOCKMGROUT_ID:STOCKMGROUT_ID,tm:new Date().getTime()},
			dataType:'json',
			cache: false,
			async: false,
			success: function(data){
	...
	function getPackRule() {
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrout/getPackRule.do',
			...
			success: function (data) {
				...
				for (var k in data) {
					...
					for (var i = 0; i < lenth; i++) {
						...
						//B.EA_OUT,B.INPACK_OUT,B.BOX_IN,B.PALLET_IN ,B.OTHER_IN
						var EA_OUT = json[i]['EA_OUT'];
						var INPACK_OUT = json[i]['INPACK_OUT'];
						var BOX_OUT = json[i]['BOX_OUT'];
						var PALLET_OUT = json[i]['PALLET_OUT'];
						var OTHER_OUT = json[i]['OTHER_OUT'];
						...
						setEA_OUT(EA_OUT,INPACK_OUT ,BOX_OUT, PALLET_OUT,OTHER_OUT );

						cleanEa();
						//OT_RATIO
						setEa(EA_OUT, INPACK_OUT, BOX_OUT, PALLET_OUT, OTHER_OUT);
						setRatio(INPACK_OUT,BOX_OUT,PALLET_OUT,OTHER_OUT);
			...
			complete: function (XMLHttpRequest, textStatus) {
			 ...
			 error: function(){
			 trs += "<tr class='main_OUTfo'><td colspan='100' class='center' >没有订单属性数据</td></tr>" ;
			 $("#table_orderproperty").append(trs);
			 }*/
		});
	...
	function setEA_OUT(EA_OUT,INPACK_OUT ,BOX_OUT, PALLET_OUT,OTHER_OUT ) {
		$("#EA_OUT_1").val(EA_OUT);
		$("#INPACK_OUT_1").val(INPACK_OUT);
		$("#BOX_OUT_1").val(BOX_OUT);
		$("#PALLET_OUT_1").val(PALLET_OUT);
		$("#OTHER_OUT_1").val(OTHER_OUT);
	}
	...
	function setRatio(INPACK_OUT,BOX_OUT,PALLET_OUT,OTHER_OUT) {
		if(INPACK_OUT ==1 && BOX_OUT !=1 && PALLET_OUT !=1 && OTHER_OUT!= 1 ) {
			$("#INPACK_RATIO").val(1);
			$("#EA_RATIO").val(1);
			return false;
		}
	...
	function setEa(EA_OUT, INPACK_OUT, BOX_OUT, PALLET_OUT, OTHER_OUT) {
		if(INPACK_OUT != 1) {
			$("#INPACK_EA").val('');
	...
	function getProdProperty() {
		...
		var auditFlag = hadProd(PRODUCT_ID,STOCKMGROUT_ID);
		//alert(auditFlag);
		if (auditFlag) {
			return false;
		}
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrout/getProdProperty.do',
...
<body>
	<form action="stockmgrout/${msg }.do" name="Form" id="Form" method="post">
		<input type="hidden" name="STOCKDTLMGROUT_ID" id="STOCKDTLMGROUT_ID" value="${pd.STOCKDTLMGROUT_ID}"/>
		<%--发货单ID--%>
		...
		<input type="hidden" name="EA_OUT_1" id="EA_OUT_1" value="${pd.EA_OUT_1}"/>
		<input type="hidden" name="INPACK_OUT_1" id="INPACK_OUT_1" value="${pd.INPACK_OUT_1}"/>
		<input type="hidden" name="BOX_OUT_1" id="BOX_OUT_1" value="${pd.BOX_OUT_1}"/>
		<input type="hidden" name="PALLET_OUT_1" id="PALLET_OUT_1" value="${pd.PALLET_OUT_1}"/>
		<input type="hidden" name="OTHER_OUT_1" id="OTHER_OUT_1" value="${pd.OTHER_OUT_1}"/>
					...
					<div class="widget-toolbox">
						<div class="btn-toolbar">
							<div class="btn-group">
								...
								<a class="btn btn-mini btn-primary" onclick="save();"><span class="icon-ok"></span>保存</a>
								</c:if>
								<a class="btn btn-mini btn-danger" onclick="top.Dialog.close();"><span
										class="icon-off"></span>关闭</a>
							</div>
						</div>
					</div>
										...
										<div class="span7">
											<table id="table_report0" class="table table-striped table-hover table-condensed">
												...
												<tr>
													...
													<%--<td><input type="text" name="PRODUCT_ID" id="PRODUCT_ID" value="${pd.PRODUCT_ID}" maxlength="32" placeholder="" title="产品"/></td>--%>
													<td  style="vertical-align:top;">
														<select  class="chzn-select"  name="PRODUCT_ID" id="PRODUCT_ID" style="vertical-align:top;" data-placeholder=" "
																 <c:if test="${not empty pd.STOCKDTLMGROUT_ID }">disabled='disabled'</c:if>
												...
												<tr>
													...
													<td>
														<input class ="input-small"  min="0" onkeyup="calcOT()"  type="number" name="OT_EA" id="OT_EA" value="${pd.OT_EA}"
															   <c:if test="${pd.OTHER_OUT_1 != 1}">readOnly='readOnly'</c:if>
																   maxlength="64"  title="其他期望数"/>
														 <input class ="input-small"  min="0" style="background-color: #d2f2e7" type="number" name="OT_RATIO" id="OT_RATIO"
																<c:if test="${pd.OTHER_OUT_1 != 1}">readOnly='readOnly'</c:if>
																value="${pd.OT_RATIO}" maxlength="32"  title="其他比例"/>
													</td>
												...
												<tr>
													...
													<td>
														<input  class ="input-small"  min="0" type="number" onkeyup="calcPALLET()"  name="PALLET_EA" id="PALLET_EA" value="${pd.PALLET_EA}" maxlength="32"
																<c:if test="${pd.PALLET_OUT_1 != 1}">readOnly='readOnly'</c:if> title="托盘期望数"/>
														<input class ="input-small"  min="0" style="background-color: #d2f2e7" type="number" name="PALLET_RATIO" id="PALLET_RATIO"
															   <c:if test="${pd.PALLET_OUT_1 != 1}">readOnly='readOnly'</c:if>
															   value="${pd.PALLET_RATIO}" maxlength="32"  title="托盘比例"/>
													</td>
												...
												<tr>
													...
													<td><input class ="input-small"  min="0" type="number" onkeyup="calcBOX()" name="BOX_EA" id="BOX_EA" value="${pd.BOX_EA}" maxlength="32"
															   <c:if test="${pd.BOX_OUT_1 != 1}">readOnly='readOnly'</c:if> title="箱期望数"/>
														<input class ="input-small"  min="0" type="number" style="background-color: #d2f2e7" name="BOX_RATIO" id="BOX_RATIO" value="${pd.BOX_RATIO}"
															   <c:if test="${pd.BOX_OUT_1 != 1}">readOnly='readOnly'</c:if> maxlength="32"  title="箱比例"/>
													</td>
												...
												<tr>
													...
													<td><input class ="input-small"  min="0" type="number" onkeyup="calcINPACK()"  name="INPACK_EA" id="INPACK_EA" value="${pd.INPACK_EA}" maxlength="32"
															   <c:if test="${pd.INPACK_OUT_1 != 1}">readOnly='readOnly'</c:if> title="内期望数"/>
												...
												<tr>
													...
													<td><input class ="input-small"  min="0" type="number" name="EA_EA" id="EA_EA" value="${pd.EA_EA}" maxlength="32" title="EA期望数"/>
													...
													<td style="width:70px;text-align: right;padding-top: 13px;">分配规则<span class="red">*</span> </td>
													<td style="vertical-align:top;">
														<select class="chzn-select" name="RULE_ASSIGNED" id="RULE_ASSIGNED" data-placeholder=" " style="vertical-align:top;">
															<option value=""></option>
															<c:forEach items="${stockAssignedRuleList}" var="cnt">
																<option value="${cnt.id }" <c:if test="${cnt.id == pd.RULE_ASSIGNED }">selected</c:if>>${cnt.value }</option>
															</c:forEach>
														</select>
													</td>
												...
												<tr>
													...
													<td style="vertical-align:top;">
														<select class="chzn-select" name="RULE_STOCKTURN" id="RULE_STOCKTURN" data-placeholder=" " style="vertical-align:top;">
															<option value=""></option>
															<c:forEach items="${stockTurnRuleList}" var="cnt">
																<option value="${cnt.id }" <c:if test="${cnt.id == pd.RULE_STOCKTURN }">selected</c:if>>${cnt.value }</option>
															</c:forEach>
														</select>
													</td>
												...
												<tr>
													...
													<td style="width:70px;text-align: right;padding-top: 13px;">拣货区</td>
												...
												<tr>
													...
													<td><input type="text" name="UNIT_PRICE" id="UNIT_PRICE" value="${pd.UNIT_PRICE}" maxlength="32" placeholder="0" title="单价"/></td>
												...
												<tr>
													<td style="width:70px;text-align: right;padding-top: 13px;">发货EA数</td>
													<td><input type="number" readOnly="readOnly" name="SEND_EA" id="SEND_EA" value="${pd.SEND_EA}" maxlength="32" placeholder="0" title="冻结拒收代码"/></td>
												...
												<tr>
													<%--<td style="width:70px;text-align: right;padding-top: 13px;">预配EA数</td>
													<td><input type="number" name="PREPLAN_EA" id="PREPLAN_EA" value="${pd.PREPLAN_EA}" maxlength="32" placeholder="0" title="冻结拒收代码"/></td>
--%>
													<td style="width:70px;text-align: right;padding-top: 13px;">分配EA数</td>
													<td><input type="number" readOnly="readOnly" name="ASSIGNED_EA" id="ASSIGNED_EA" value="${pd.ASSIGNED_EA}" maxlength="32" placeholder="0" title=""/></td>

													<td style="width:70px;text-align: right;padding-top: 13px;">拣货EA数</td>
													<td><input type="number" readOnly="readOnly" name="PICK_EA" id="PICK_EA" value="${pd.PICK_EA}" maxlength="32" placeholder="0" title="拣货EA数"/></td>

												</tr>
												<tr>

													<td style="width:70px;text-align: right;padding-top: 13px;">装车EA数</td>
													<td><input type="number" readOnly="readOnly" name="LOADED_EA" id="LOADED_EA" value="${pd.LOADED_EA}" maxlength="32" placeholder="0" title="装车EA数"/></td>
										...
										<div class="span5">
											...
											<div class="widget-box ">
												<div class="widget-body">
													<div class="widget-main  padding-3">
														<div class="slim-scroll" data-height="890">
															<div class="content">
																<table id="table_orderproperty"
																	...
																	<c:choose>
																			...
																			<c:if test="${QX.cha == 1 }">
																				<c:forEach items="${inOrderList}" var="var"
																					...
																					<tr>
																						...
																						<td tyle="width: 120px;">${var.PROPERTY_KEY}</td>
																						<td class="input-sm">
																		...
																		<c:otherwise>
																			<tr class="main_OUTfo">
																				<td colspan="100" class="center">没有批次属性数据
																				</td>
																			</tr>
																		</c:otherwise>
																	</c:choose>
	...
	<script type="text/javascript" src="static/js/jquery.slimscroll.min.js"></script>
		...
		function calcBOX() {
			var EA_OUT_1 = $("#EA_OUT_1").val();
			var INPACK_OUT_1 = $("#INPACK_OUT_1").val();
			var BOX_OUT_1 = $("#BOX_OUT_1").val();
			...
			if (BOX_OUT_1 == 1 && INPACK_OUT_1 == 1 ) {
				calc21(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1);
				return false;
			}

			if (BOX_OUT_1 == 1 ) {
				calc2(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1);
				return false;
			}

		}

		function calc2(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1) {
			var cnt = $("#BOX_EA").val();
			$("#EA_EA").val(cnt*BOX_NUM_1*EA_NUM_1);
		}

		function calc21(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1) {
			var cnt = $("#BOX_EA").val();
		...
		function calcPALLET() {
			...
			var INPACK_OUT_1 = $("#INPACK_OUT_1").val();
			var BOX_OUT_1 = $("#BOX_OUT_1").val();
			var PALLET_OUT_1 = $("#PALLET_OUT_1").val();
			...
			if (PALLET_OUT_1 == 1&& BOX_OUT_1 == 1&& INPACK_OUT_1 == 1 ) {
				calc321(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1);
				return false;
			}

			if (PALLET_OUT_1 == 1&& BOX_OUT_1 == 1) {
				calc32(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1);
				return false;
			}

			if (PALLET_OUT_1 == 1&& INPACK_OUT_1 == 1 ) {
				calc31(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1);
				return false;
			}

			if (PALLET_OUT_1 == 1) {
				calc3(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1);
				return false;
			}
		}

		function calc3(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1) {
			//第一级根据其他算托盘数量
			var cnt = $("#PALLET_EA").val();
			$("#EA_EA").val(cnt*PALLET_NUM_1*EA_NUM_1);
		}

		function calc31(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1) {
			//第一级根据其他算托盘数量
			var cnt = $("#PALLET_EA").val();
		...
		function calc32(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1) {
			//第一级根据其他算托盘数量
			var cnt = $("#PALLET_EA").val();
		...
		function calc321(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1) {
			//第一级根据其他算托盘数量
			var cnt = $("#PALLET_EA").val();
			$("#BOX_EA").val(cnt*PALLET_NUM_1);
		...
		function calcOT() {
			...
			var BOX_OUT_1 = $("#BOX_OUT_1").val();
			var PALLET_OUT_1 = $("#PALLET_OUT_1").val();
			var OTHER_OUT_1 = $("#OTHER_OUT_1").val();
			...
			if (OTHER_OUT_1 == 1 && PALLET_OUT_1 == 1&& BOX_OUT_1 == 1&& INPACK_OUT_1 == 1 ) {
				calcOT4321(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

			if (OTHER_OUT_1 == 1 && PALLET_OUT_1 == 1&& BOX_OUT_1 == 1) {
				calcOT432(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

			if (OTHER_OUT_1 == 1 && PALLET_OUT_1 == 1&& INPACK_OUT_1 == 1 ) {
				calcOT431(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

			if (OTHER_OUT_1 == 1 && BOX_OUT_1 == 1&& INPACK_OUT_1 == 1 ) {
				calcOT421(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

			if (OTHER_OUT_1 == 1 && PALLET_OUT_1 == 1 ) {
				calcOT43(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

			if (OTHER_OUT_1 == 1 && BOX_OUT_1 == 1 ) {
				calcOT42(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

			if (OTHER_OUT_1 == 1 &&  INPACK_OUT_1 == 1 ) {
				calcOT41(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

			if (OTHER_OUT_1 == 1) {
				calcOT4(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1);
				return false;
			}

		}

		function calcOT4(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			//第一级根据其他算托盘数量
			var ot_ea = $("#OT_EA").val();
			$("#EA_EA").val(ot_ea*OTHER_NUM_1*EA_NUM_1);
		}

		function calcOT41(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			//第一级根据其他算托盘数量
			var ot_ea = $("#OT_EA").val();
		...
		function calcOT42(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			//第一级根据其他算托盘数量
			var ot_ea = $("#OT_EA").val();
		...
		function calcOT43(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			//第一级根据其他算托盘数量
			var ot_ea = $("#OT_EA").val();
		...
		function calcOT421(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			//第一级根据其他算托盘数量
			var ot_ea = $("#OT_EA").val();
			$("#BOX_EA").val(ot_ea*OTHER_NUM_1);
		...
		function calcOT431(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			//第一级根据其他算托盘数量
			var ot_ea = $("#OT_EA").val();
			$("#PALLET_EA").val(ot_ea*OTHER_NUM_1);
		...
		function calcOT4321(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			//第一级根据其他算托盘数量
			var ot_ea = $("#OT_EA").val();
			$("#PALLET_EA").val(ot_ea*OTHER_NUM_1);
		...
		function calcOT432(EA_OUT_1,INPACK_OUT_1,BOX_OUT_1,PALLET_OUT_1,OTHER_OUT_1,EA_NUM_1,INPACK_NUM_1,BOX_NUM_1,PALLET_NUM_1,OTHER_NUM_1) {
			var ot_ea = $("#OT_EA").val();
			$("#PALLET_EA").val(ot_ea*OTHER_NUM_1);
			$("#BOX_EA").val(ot_ea*OTHER_NUM_1*PALLET_NUM_1);
