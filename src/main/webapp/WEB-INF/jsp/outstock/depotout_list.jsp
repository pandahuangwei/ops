...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="depotout/list.do" method="post" name="Form" id="Form">
						...
						<div class="widget-toolbox padding-8 clearfix">
							<div class="btn-group">
								<button class="btn btn-primary btn-mini" onclick="search();" title="检索" data-rel="tooltip">
									...
									<c:if test="${QX.add == 1 }">
										<a class="btn btn-mini btn-info"  onclick="manualDepot(1);"><span class="icon-ok"></span>发货</a>
									</c:if>
								<c:if test="${QX.add == 1 }">
      ...
      //LOADED_TYPE_S  ASSIGNED_STATE_S DEPOT_STATE_S  PICK_STATE_S  OUTSTOCK_TYPE_S OUTSTOCK_WAY_S
		...
		function manualDepot(opttype) {
			...
			if (str == '') {
				...
				diag.Drag = true;
				diag.Title = title1;
				diag.URL = '<%=basePath%>depotout/goDepotManual.do?DEPOT_TYPE=' + opttype +"&STOCKMGROUT_IDS="+str;
				diag.Width = 1100;
				diag.Height = 800;
		...
		function cancelDepot(opttype) {
			...
			if (str == '') {
				...
				diag.Drag = true;
				diag.Title = title1;
				diag.URL = '<%=basePath%>depotout/goDepotCacel.do?DEPOT_TYPE=' + opttype +"&STOCKMGROUT_IDS="+str;
				diag.Width = 1000;
				diag.Height = 800;
				diag.CancelEvent = function () { //关闭事件
					$("#Form").submit();
		...
		function look(Id){
			...
			diag.Drag=true;
			diag.Title ="查看发货明细";
			diag.URL = '<%=basePath%>depotout/goDepotLook.do?STOCKMGROUT_ID='+Id;
			diag.Width = 1000;
			diag.Height = 800;
			diag.CancelEvent = function(){ //关闭事件
