   ...
   <script type="text/javascript">
	...
	function save(pid){
		...
		if (str == '') {
			...
			$('#Form').attr("action", "assignout/saveAuto.do?OPT_EVEN="+pid+"&STOCKMGROUT_ID_CHOSE="+str).submit();
	...
	<form action="assignout/goAssignAuto.do" name="Form" id="Form" method="post">
											...
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${varList}" var="var" varStatus="vs">
													...
													<%--	<td <c:if test="${133==var.LOADED_STATE}">style="background-color: red" </c:if>
														...
														<td class="right" type="number">${var.EA_EA}</td>
														<td class="right" type="number">${var.ASSIGNED_EA}</td>
															<%--超量分配 OVERLOAD --%>
													<%--	<td <c:if test="${var.OVERLOAD < 0 }">style="background-color: pink" </c:if> >
				...
				<div class="widget-main padding-3">
					<div> <%-- class="slim-scroll" data-height="125"--%>
						<div class="content">
							...
							<table id="table_prod_dtl2" class="table table-striped table-bordered table-hover">
