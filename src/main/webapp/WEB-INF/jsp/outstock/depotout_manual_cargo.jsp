   ...
   <script type="text/javascript">
	...
	function manualDepot(pid) {
		...
		if (str == '') {
			...
			$('#Form').attr("action", "depotout/saveDepotCargo.do?OPT_EVEN="+pid+"&STOCKMGROUT_ID_CHOSE="+str).submit();
	...
	function manualCancel(pid) {
		...
		if (str == '') {
			...
			$('#Form').attr("action", "depotout/saveCancelManual.do?OPT_EVEN="+pid+"&STOCKMGROUT_ID_CHOSE="+str).submit();
	...
	<form action="depotout/goDepotCargoDepot.do" name="Form" id="Form" method="post">
		<input type="hidden" name="CARGOOUT_ID" id="CARGOOUT_ID" value="${pd.CARGOOUT_ID}"/>
		<input type="hidden" name="DEPOT_TYPE" id="DEPOT_TYPE" value="${pd.DEPOT_TYPE}"/>
				...
				<div class="widget-body">
					...
					<div class="widget-toolbox">
						<div class="btn-toolbar">
							<div class="btn-group">
								<a class="btn btn-mini btn-primary" onclick="manualDepot(1);"><span class="icon-ok"></span>整单发货</a>
							</div>
											...
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${varList}" var="var" varStatus="vs">
													<tr>
														...
														<%--分配状态--%>
														<td <c:if test="${161==var.CARGO_STATE_CARGO}">style="background-color: red" </c:if>
															<c:if test="${162==var.CARGO_STATE_CARGO}">style="background-color: yellow" </c:if>
															<c:if test="${163==var.CARGO_STATE_CARGO}">style="background-color: green" </c:if>>
															<c:forEach items="${cargoStateList}" var="cnt">
																<c:if test="${cnt.id==var.CARGO_STATE_CARGO}">${cnt.value}</c:if>
															</c:forEach>
														</td>


														<td style="text-align: right">${var.CARGO_TM}</td>
														<td style="text-align: right">${var.DEPOT_TM}</td>
														<td style="text-align: right">${var.CARGO_EA_TOTAL}</td>
													</tr>
			...
			<div class="widget-box">
				...
				<div class="widget-body">
					...
					<div class="widget-toolbox">
						<div class="btn-toolbar">
							<div class="btn-group">
								<a class="btn btn-mini btn-primary" onclick="manualDepot2(2);"><span class="icon-ok"></span>明细发货</a>
							</div>
										...
										<c:if test="${QX.cha == 1 }">
											<c:forEach items="${assignList}" var="var" varStatus="vs">
												<tr>
													...
													<td class='center' style="width: 30px;">
														<label><input type='checkbox' name='ids1' value="${var.COMB_IDS}" /><span class="lbl"></span></label>
													</td>
