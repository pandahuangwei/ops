						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								<td class='center' style="width: 30px;">
									<c:if test="${user.USERNAME != 'administrator'}"><label><input type='checkbox' name='ids' value="${user.USER_ID }" id="${user.EMAIL }" alt="${user.PHONE }"/><span class="lbl"></span></label></c:if>
									<c:if test="${user.USERNAME == 'administrator'}"><label><input type='checkbox' disabled="disabled" /><span class="lbl"></span></label></c:if>
								</td>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit == 1 }">
											<c:if test="${user.USERNAME != 'administrator'}"><a class='btn btn-mini btn-info' title="编辑" onclick="editUser('${user.USER_ID }');"><i class='icon-edit'></i></a></c:if>
											<c:if test="${user.USERNAME == 'administrator'}"><a class='btn btn-mini btn-info' title="您不能编辑"><i class='icon-edit'></i></a></c:if>
										</c:if>
										<c:choose>
											<c:when test="${user.USERNAME=='administrator'}">
