...
<script type="text/javascript">
	...
	function scanput() {
		...
		diag.Drag = true;
		diag.Title = "扫描转移";
		diag.URL = '<%=basePath%>locatortf/goScanPut.do';
		diag.Width = 1000;
		diag.Height = 800;
	...
	<div id="page-content" class="clearfix">
		...
		<div class="row-fluid">
			...
			<div class="row-fluid">
				...
				<!-- 检索  -->
				<form action="locatortf/list.do" method="post" name="Form" id="Form">
							...
							<div class="widget-toolbox padding-8 clearfix">
								<div class="btn-group">
									...
									<c:if test="${QX.add == 1 }">
										...
										<a class="btn btn-mini btn-info"  onclick="scanput();"><span class="icon-camera-retro"></span>扫描转移</a>
									</c:if>
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr id="${var.STOCKDTLMGRIN_ID}">
											...
											<td style="background-color: lightyellow">
												<c:forEach items="${locatorList}" var="cnt">
													<c:if test="${cnt.id==var.PUT_LOCATOR_BF}">${cnt.value}</c:if>
												</c:forEach>
											</td>
...
<script type="text/javascript">
	...
	function transfer() {
		...
		if (str == '') {
			...
			diag.Drag = true;
			diag.Title = title1;
			diag.URL = '<%=basePath%>locatortf/goLocatorTf.do?STOCKDTLMGRIN_IDS='+str;
			diag.Width = 800;
			diag.Height = 300;
			diag.CancelEvent = function () { //关闭事件
				$("#Form").submit();
