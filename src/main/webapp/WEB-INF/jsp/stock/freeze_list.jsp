...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="freeze/list.do" method="post" name="Form" id="Form">
						...
						<div class="widget-toolbox padding-8 clearfix">
							<div class="btn-group">
								<button class="btn btn-primary btn-mini" onclick="search();" title="查询" data-rel="tooltip">
									...
									<c:if test="${QX.add == 1 }">
										<a class="btn btn-mini btn-success" onclick="freeze('确定要冻结选中的数据吗?');"><span class="icon-ok"></span>冻结</a>
									</c:if>
								<c:if test="${QX.del == 1 }">
									<a class="btn btn-mini btn-warning" onclick="free('确定要释放选中的数据吗?');"><span class="icon-undo"></span>释放</a>
								</c:if>
				...
				if(result) {
					...
					if(str==''){
						...
						if (msg == '确定要冻结选中的数据吗?') {
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>freeze/saveFreeze.do?tm=' + new Date().getTime(),
								data: {STOCKDTLMGRIN_IDS: str},
								dataType: 'json',
				...
				if(result) {
					...
					if(str==''){
						...
						if (msg == '确定要释放选中的数据吗?') {
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>freeze/saveFree.do?tm=' + new Date().getTime(),
								data: {STOCKDTLMGRIN_IDS: str},
								dataType: 'json',
								//beforeSend: validateData,
								cache: false,
