...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="batchModify/list2.do" method="post" name="Form" id="Form">
							...
							<div class="slim-scroll" data-height="125">
								<div class="content">
									...
									<table id="table_prod_dtl" class="table table-striped table-condensed table-hover">
										...
										<tr>
											<td style="width:110px;text-align: right;padding-top: 13px;">修改批次号</td>
											<td><input  id="MODIFY_BATCH" type="text" name="MODIFY_BATCH" value="${pd.MODIFY_BATCH}" /></td>
											<%--<td><select  class="chzn-select" name="MODIFY_BATCH" id="MODIFY_BATCH" style="vertical-align:top;" data-placeholder=" ">
												<option value=""></option>
												<c:forEach items="${batchModifyList}" var="cnt">
													<option value="${cnt.id }"
															<c:if test="${cnt.id == pd.MODIFY_BATCH }">selected</c:if>>${cnt.value }</option>
												</c:forEach>
											</select>
											</td>--%>

											<td style="width:70px;text-align: right;padding-top: 13px;">修改日期</td>
											<td colspan="7">
													<input class="date-picker" name="MODIFY_TM_BEGIN"
													   id="MODIFY_TM_BEGIN" value="${pd.MODIFY_TM_BEGIN}" type="text"
													   data-date-format="yyyy-mm-dd" readonly="readonly"
													   placeholder=""/>
												到
												<input class="date-picker" name="MODIFY_TM_END" id="MODIFY_TM_END"
													   value="${pd.MODIFY_TM_END}" type="text"
													   data-date-format="yyyy-mm-dd" readonly="readonly"
													   placeholder=""/>
											</td>
										...
										<tr>
											...
											<td  colspan="7" style="text-align: left;padding-top: 13px;">
												<span class="lbl">显示修改/前Remark1</span>
												<input  class="input-small" type='checkbox' id="OLD_REMARK1_SHOW"  name='OLD_REMARK1_SHOW' value="1"
														<c:if test="${pd.OLD_REMARK1_SHOW eq 1 }">checked="checked" </c:if>/><span class="lbl"></span>
												<span class="lbl">前Remark2</span>
												<input  class="input-small" type='checkbox' id="OLD_REMARK2_SHOW"  name='OLD_REMARK2_SHOW' value="1"
														<c:if test="${pd.OLD_REMARK2_SHOW eq 1 }">checked="checked" </c:if>/><span class="lbl"></span>
												<span class="lbl">前Remark3</span>
												<input  class="input-small" type='checkbox' id="OLD_REMARK3_SHOW"  name='OLD_REMARK3_SHOW' value="1"
														<c:if test="${pd.OLD_REMARK3_SHOW eq 1 }">checked="checked" </c:if>/><span class="lbl"></span>
											</td>
										<%--	<td  colspan="7" style="text-align: left;padding-top: 13px;">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">前Remark1</th>
						<th class="center">Remark1</th>
						<th class="center">前Remark2</th>
						<th class="center">Remark2</th>
						<th class="center">前Remark3</th>
						<th class="center">Remark3</th>
						<th class="center">修改批次号</th>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr id="${var.P_ID}">
								...
								<td>${var.DATE_CODE}</td>
								<td>${var.LOT_NO}</td>
								<td>${var.OLD_REMARK1}</td>
								<td <c:if test="${1==var.REMARK1_M}">style="background-color: lightgreen" </c:if>>${var.REMARK1}</td>
								<td>${var.OLD_REMARK2}</td>
								<td <c:if test="${1==var.REMARK2_M}">style="background-color: lightgreen" </c:if>>${var.REMARK2}</td>
								<td>${var.OLD_REMARK3}</td>
								<td <c:if test="${1==var.REMARK3_M}">style="background-color: lightgreen" </c:if>>${var.REMARK3}</td>
								<td>${var.MODIFY_BATCH}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		$(function() {
			...
			$('#MODIFY_TM_BEGIN').datepicker({
				...
				$('#MODIFY_TM_END').datepicker('setStartDate',startTime);
			});
			//结束时间：
			$('#MODIFY_TM_END').datepicker({
				todayBtn : "linked",
				autoclose : true,
				todayHighlight : true
				//endDate : new Date()
				...
				var endTime = e.date;
				$('#MODIFY_TM_BEGIN').datepicker('setEndDate',endTime);
			});

			$('#OLD_REMARK1_SHOW').on('click', function() {
				if ($(this).prop('checked')) {
					$('tr').find('th:eq(11)').show();
					$('tr').find('td:eq(11)').show();
				} else {
					$('tr').find('th:eq(11)').hide();
					$('tr').find('td:eq(11)').hide();
				}
			});
			$('#OLD_REMARK2_SHOW').on('click', function() {
				if ($(this).prop('checked')) {
					$('tr').find('th:eq(13)').show();
					$('tr').find('td:eq(13)').show();
				} else {
					$('tr').find('th:eq(13)').hide();
					$('tr').find('td:eq(13)').hide();
				}
			});

			$('#OLD_REMARK3_SHOW').on('click', function() {
				if ($(this).prop('checked')) {
			...
			if ($('#OLD_REMARK1_SHOW').prop('checked')) {
				$('tr').find('th:eq(11)').show();
				$('tr').find('td:eq(11)').show();
			}else {
				$('tr').find('th:eq(11)').hide();
			...
			if ($('#OLD_REMARK2_SHOW').prop('checked')) {
				$('tr').find('th:eq(13)').show();
				$('tr').find('td:eq(13)').show();
			}else {
				$('tr').find('th:eq(13)').hide();
			...
			if ($('#OLD_REMARK3_SHOW').prop('checked')) {
				$('tr').find('th:eq(15)').show();
				$('tr').find('td:eq(15)').show();
			}else {
				$('tr').find('th:eq(15)').hide();
