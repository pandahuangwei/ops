...
<script type="text/javascript">
			...
			function scanput() {
				...
				diag.Drag = true;
				diag.Title = "扫描码放";
				diag.URL = '<%=basePath%>putlocatoropt/goScanPut.do';
				diag.Width = 1000;
				diag.Height = 800;
			...
			function scanput2() {
				...
				if (str == '') {
					...
					diag.Drag = true;
					diag.Title = "扫描码放";
					diag.URL = '<%=basePath%>putlocatoropt/goScanPut.do?INSTOCK_NOS='+str;
					diag.Width = 1000;
					diag.Height = 800;
					diag.CancelEvent = function () { //关闭事件
						if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="putlocatoropt/list.do" method="post" name="Form" id="Form">
						...
						<div class="widget-toolbox padding-8 clearfix">
							<div class="btn-group">
								...
								<c:if test="${QX.add == 1 }">
									...
									<a class="btn btn-mini btn-info"  onclick="scanput();"><span class="icon-camera-retro"></span>扫描码放</a>

									<a class="btn btn-mini btn-primary" onclick="goManualPut();"><span class="icon-eye-open"></span>手工码放</a>

									<a class="btn btn-mini btn-warning" onclick="goCancel();"><span class="icon-undo"></span>取消码放</a>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function goManualPut() {
			...
			if (str == '') {
				...
				diag.Drag = true;
				diag.Title = "手工码放";
				diag.URL = '<%=basePath%>putlocatoropt/goManualPut.do?INSTOCK_NOS='+str;
				diag.Width = 1200;
				diag.Height = 800;
				diag.CancelEvent = function () { //关闭事件
					/*if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
		...
		function goCancel() {
			...
			if (str == '') {
				...
				diag.Drag = true;
				diag.Title = "手工码放";
				diag.URL = '<%=basePath%>putlocatoropt/goManualCacel.do?INSTOCK_NOS='+str;
				diag.Width = 1200;
				diag.Height = 800;
				diag.CancelEvent = function () { //关闭事件
					$("#Form").submit();
		...
		function look(Id){
			...
			diag.Drag=true;
			diag.Title ="查看码放明细";
			diag.URL = '<%=basePath%>putlocatoropt/goLook.do?STOCKMGRIN_ID='+Id;
			diag.Width = 1200;
			diag.Height = 800;
			diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
