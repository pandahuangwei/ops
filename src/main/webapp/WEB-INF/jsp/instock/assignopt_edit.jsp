   ...
   <script type="text/javascript">
	...
	function saveHead(pid){
		...
		if (str == '') {
			...
			$('#Form').attr("action", "assignopt/saveHead.do?OPT_EVEN="+pid+"&INSTOCK_NO_CHOSE="+str).submit();
	...
	<form action="assignopt/goAssign.do" name="Form" id="Form" method="post">
			...
			<div class="widget-box">
				<div class="widget-header widget-hea1der-small">
					<h6><strong> ${pd.ASSIGN_NAME}</strong>
						<c:if test="${pd !=null and  pd.AUTO_FLG != null}">
							${pd.AUTO_FLG}
						</c:if>
					</h6>
				...
				<div class="widget-body">
					...
					<div class="widget-toolbox">
						<div class="btn-toolbar">
							<div class="btn-group">
								<c:if test="${pd !=null and pd.ASSIGN_TYPE != 4 and  (pd.AUTO_FLG == null or pd.AUTO_FLG == '')}">
									<c:if test="${pd.ASSIGN_TYPE == 1}">
										<a class="btn btn-mini btn-primary" onclick="saveHead('${pd.ASSIGN_TYPE}');"><span class="icon-cogs"></span>自动分配-整单</a>
									</c:if>
									<c:if test="${pd.ASSIGN_TYPE == 2}">
										<a class="btn btn-mini btn-primary" onclick="saveHead('${pd.ASSIGN_TYPE}');"><span class="icon-eye-open"></span>手工分配-整单</a>
									</c:if>
									<c:if test="${pd.ASSIGN_TYPE == 3}">
										<a class="btn btn-mini btn-primary" onclick="saveHead('${pd.ASSIGN_TYPE}');"><span class="icon-eye-close"></span>缺省分配-整单</a>
									</c:if>
								</c:if>
								<c:if test="${pd !=null and pd.ASSIGN_TYPE == 4 }">
									<a class="btn btn-mini btn-warning" onclick="saveHead('${pd.ASSIGN_TYPE}');"><span class="icon-undo"></span>取消分配-整单</a>
								</c:if>
							</div>
							&nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp;
							&nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp;
							...
							<c:if test="${pd !=null and pd.ASSIGN_TYPE != 4 and  (pd.AUTO_FLG == null or pd.AUTO_FLG == '')}">
								<c:if test="${pd.ASSIGN_TYPE == 2}">
									<table id="table_report2" class="table table-striped  table-hover table-condensed">
										<tr>
											<td style="width:70px;text-align: right;padding-top: 13px;">收货库区<span class="red">*</span></td>
											...
											<td style="width:70px;text-align: right;padding-top: 13px;">收货库位<span class="red">*</span></td>
											<td style="vertical-align:top;">
					...
					<div class="widget-toolbox">
						<div class="btn-toolbar">
							<div class="btn-group">
								<c:if test="${pd !=null and pd.ASSIGN_TYPE != 4 and  (pd.AUTO_FLG == null or pd.AUTO_FLG == '')}">
									<c:if test="${pd.ASSIGN_TYPE == 1}">
										<a class="btn btn-mini btn-primary" onclick="save('${pd.ASSIGN_TYPE}');"><span class="icon-cogs"></span>自动分配-明细</a>
									</c:if>
									<c:if test="${pd.ASSIGN_TYPE == 2}">
										<a class="btn btn-mini btn-primary" onclick="save('${pd.ASSIGN_TYPE}');"><span class="icon-eye-open"></span>手工分配-明细</a>
									</c:if>
									<c:if test="${pd.ASSIGN_TYPE == 3}">
										<a class="btn btn-mini btn-primary" onclick="save('${pd.ASSIGN_TYPE}');"><span class="icon-eye-close"></span>缺省分配-明细</a>
									</c:if>
								</c:if>
								<c:if test="${pd !=null and pd.ASSIGN_TYPE == 4 }">
									<a class="btn btn-mini btn-warning" onclick="save('${pd.ASSIGN_TYPE}');"><span class="icon-undo"></span>取消分配-明细</a>
								</c:if>
							</div>
						</div>
