  ...
  <%--  <script type="text/javascript" src="static/js/jquery-1.7.2.js"></script>
    ...
    <script type="text/javascript">
        ...
        function save(pid) {
           ...
           // action="stockmgrinsch/${msg}.do"
            var saveEven = $("#MSG_ID").val();
           // alert(saveEven);
            //保存，并停在当前页面
            if (saveEven == "save") {
                if (pid == 1) {
                    $('#Form').attr("action", "stockmgrinsch/save.do?OPT_EVEN="+pid).submit();
                } else {
                    $('#Form').attr("action", "stockmgrinsch/save.do?OPT_EVEN="+pid).submit();
                }
            }
            ...
            if (saveEven == "edit") {
                if (pid == 1) {
                    $('#Form').attr("action", "stockmgrinsch/edit.do?OPT_EVEN="+pid).submit();
                } else {
                    $('#Form').attr("action", "stockmgrinsch/edit.do?OPT_EVEN="+pid).submit();
                }
            }
        ...
        function getInStockNo() {
            ...
            $.ajax({
                type: "POST",
                url: '<%=basePath%>stockmgrinsch/getInStockNo.do',
        ...
        function customerChange() {
            ...
            $.ajax({
                type: "POST",
                url: '<%=basePath%>stockmgrinsch/getOrderProperty.do',
                data: {CUSTOMER_ID: CUSTOMER_ID, tm: new Date().getTime()},
                dataType: 'json',
                cache: false,
                success: function (data) {
                                        ...
                                        <c:if test="${QX.cha == 1 }">
                                            <c:forEach items="${varList}" var="var" varStatus="vs">
                                                <tr id="${var.STOCKDTLMGRIN_ID}">
                                                    ...
                                                    <td  name='products' values= "${var.PRODUCT_ID}">${var.PRODUCT_CN}</td>
                                                    <td>${var.SCAN_CODE}</td>
                                                    <td>${var.EA_EA}</td>
                                                    <td>${var.BOX_NO}</td>
                                                    <td>${var.EA_ACTUAL}</td>
...
<script type="text/javascript">
    function addDtl() {
       ...
       // alert(CUSTOMER_ID);
        ...
        diag.Drag = true;
        diag.Title = " 新增入库单明细";
        diag.URL = '<%=basePath%>stockmgrinsch/goAddDtl.do?CUSTOMER_ID='+CUSTOMER_ID+"&ROW_NO="+ROW_NO+"&STOCKMGRIN_ID="+STOCKMGRIN_ID+"&tm="+new Date().getTime()+"&HAD_PRODS="+str;
    ...
    function refreshDtl() {
        $('#Form').attr("action", "stockmgrinsch/edit.do?OPT_EVEN="+"").submit();
        $("#zhongxin").hide();
        $("#zhongxin2").show();
    }
    ...
    function refreshDtl2() {
        ...
        $.ajax({
            type: "POST",
            url: '<%=basePath%>stockmgrinsch/getProdDtl.do',
    ...
    function del(Id){
        bootbox.confirm("确定要删除吗?", function(result) {
            if(result) {
                //top.jzts();
                var url = "<%=basePath%>stockmgrinsch/deleteDtl.do?STOCKDTLMGRIN_ID="+Id+"&tm="+new Date().getTime();
    ...
    function edit(Id){
        ...
        diag.Drag = true;
        diag.Title ="编辑入库单明细";
        diag.URL = '<%=basePath%>stockmgrinsch/goEditDtl.do?STOCKDTLMGRIN_ID='+Id+"&STOCKMGRIN_ID="+STOCKMGRIN_ID;
        diag.Width = 1100;
        diag.Height = 800;
        diag.CancelEvent = function(){ //关闭事件
            if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
    ...
    function upload(){
        ...
        diag.URL = '<%=basePath%>stockmgrinsch/goUploadExcel.do?STOCKMGRIN_ID='+STOCKMGRIN_ID+"&&CUSTOMER_ID="+CUSTOMER_ID;
        diag.Width = 700;
        diag.Height = 450;
        diag.CancelEvent = function(){ //关闭事件
            ...
            if(result) {
                ...
                if(str==''){
                    ...
                    if(msg == '确定要删除选中的数据吗?'){
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>stockmgrinsch/deleteDtlAll.do?tm='+new Date().getTime(),
                            data: {DATA_IDS:str},
                            dataType:'json',
