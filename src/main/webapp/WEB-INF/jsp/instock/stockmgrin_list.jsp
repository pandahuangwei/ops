...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="stockmgrin/list.do" method="post" name="Form" id="Form">
							...
							<%--<div style="overflow-y:auto; overflow-x:auto; ”>--%>
								<div class="content">
									...
									<table id="table_prod_dtl" class="table table-striped table-condensed table-hover">
										...
										<tr>
											<td style="width:110px;text-align: right;padding-top: 13px;">Inbound Reference</td>
										...
										<tr>
											...
											<td>
												<select class="chzn-select" name="COST_STATE" id="COST_STATE" data-placeholder="请选择" >
													<option value="">全部</option>
													<option value="1" <c:if test="${'1' eq pd.COST_STATE }">selected</c:if>>已完成</option>
													<option value="0"  <c:if test="${'0' eq pd.COST_STATE }">selected</c:if>>未完成</option>
												</select>
											</td>
										...
										<tr>
											...
											<td colspan="7"  style="vertical-align:top;">
												<select class="chzn-select"  name="SORT_CLOUM" id="SORT_CLOUM"
													...
													<option value="SHIPPER" <c:if test="${'SHIPPER' eq pd.SORT_CLOUM }">selected</c:if>>Shipper</option>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>stockmgrin/delete.do?STOCKMGRIN_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>stockmgrin/goEdit.do?STOCKMGRIN_ID='+Id;
			 diag.Width = 1100;
			 diag.Height = 800;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>stockmgrin/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>stockmgrin/excel.do';
		}
		</script>
