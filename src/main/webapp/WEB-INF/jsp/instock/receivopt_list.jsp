...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="receivopt/list.do" method="post" name="Form" id="Form">
											...
											<td colspan="3">
											   ...
											   <td colspan="3"  style="vertical-align:top;">
												   <select class="chzn-select"  name="SORT_CLOUM" id="SORT_CLOUM"
													   ...
													   <option value=""></option>
													   <option value="INSTOCK_NO" <c:if test="${'INSTOCK_NO' eq pd.SORT_CLOUM }">selected</c:if>>入库单</option>
													   <option value="INBOUND_REFERENCE" <c:if test="${'INBOUND_REFERENCE' eq pd.SORT_CLOUM }">selected</c:if>>Inbound Reference</option>
													   <option value="INSTOCK_TYPE" <c:if test="${'INSTOCK_TYPE' eq pd.SORT_CLOUM }">selected</c:if>>入库类型</option>
													   <option value="PRE_INSTOCK_DATE" <c:if test="${'PRE_INSTOCK_DATE' eq pd.SORT_CLOUM }">selected</c:if>>预计入库日期</option>
													   <option value="REAL_INSTOCK_DATE" <c:if test="${'REAL_INSTOCK_DATE' eq pd.SORT_CLOUM }">selected</c:if>>实际入库日期</option>
												   </select>
						...
						<div class="widget-toolbox padding-8 clearfix">
							<div class="btn-group">
								...
								<c:if test="${QX.add == 1 }">
									...
									<a class="btn btn-mini btn-success"  onclick="goAddCommon('COMMON');"><span class="icon-plus"></span>通用收货</a>

									<a class="btn btn-mini btn-info"  onclick="add('REOTEC');"><span class="icon-plus"></span>REOTEC收货</a>

									<a class="btn btn-mini btn-primary" onclick="addSprea('SPREA');"><span class="icon-plus"></span>SPREA收货</a>
									<%--<c:if test="${QX.add == 5 }">--%>
									<a class="btn btn-mini btn-primary"  onclick="upload('SPREA');"  title="从EXCEL导入"><span class="icon-cloud-upload"></span>SPREA收货导入</a>
									<%--</c:if>--%>
			...
			<!-- 检索  -->
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					...
					<tr>
						...
						<th class="center">Inbound Reference</th>
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr id="${var.STOCKMGRIN_ID}">
										<td class='center' style="width: 30px;">
											<label><input type='checkbox' name='ids' value="${var.STOCKMGRIN_ID}" /><span class="lbl"></span></label>
										</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function goAddCommon(opttype){
			top.jzts();
			var diag = new top.Dialog();
			diag.Drag=true;
			diag.Title ="通用收货";
			diag.URL = '<%=basePath%>receivopt/goAddCommon.do?RECEIV_TYPE='+opttype;
			diag.Width = 1200;
			diag.Height = 800;
		...
		function add(opttype){
			 ...
			 diag.Drag=true;
			 diag.Title ="Reotec收货";
			 diag.URL = '<%=basePath%>receivopt/goAdd.do?RECEIV_TYPE='+opttype;
			 diag.Width = 1200;
			 diag.Height = 800;
			 diag.CancelEvent = function(){ //关闭事件
				 /*if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//新增
		function addSprea(opttype){
			top.jzts();
			var diag = new top.Dialog();
			diag.Drag=true;
			diag.Title ="Sprea收货";
			diag.URL = '<%=basePath%>receivopt/goAddSprea.do?RECEIV_TYPE='+opttype;
			diag.Width = 1200;
			diag.Height = 800;
			diag.CancelEvent = function(){ //关闭事件
				$("#Form").submit();
		...
		function look(Id){
			...
			diag.Drag=true;
			diag.Title ="查看收货明细";
			diag.URL = '<%=basePath%>receivopt/goLook.do?STOCKMGRIN_ID='+Id;
			diag.Width = 1200;
			diag.Height = 800;
			diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function upload(opttype){
			top.jzts();
			var diag = new top.Dialog();
			diag.Drag=true;
			diag.Title ="Sprea收货导入";
			diag.URL = '<%=basePath%>receivopt/goSpreaExcel.do?opttype='+opttype;
			diag.Width = 700;
			diag.Height = 450;
			diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
