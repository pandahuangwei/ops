  ...
  <div class="row-fluid">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">前缀料号</th>
						<th class="center">料号</th>
						<th class="center">前缀数量</th>
						<th class="center">数量</th>
						<th class="center">COO</th>
						<th class="center">PACKAGE QTY</th>
						<th class="center">板号</th>
						<th class="center">二维码</th>
						<th class="center">前缀LOTNO</th>
						<th class="center">LOT_NO</th>
						<th class="center">前缀DATECODE</th>
						<th class="center">DATECODE</th>
						<th class="center">前缀扫描码数量</th>
						<th class="center">扫描码</th>
						<th class="center">描码数量</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${var.RECEIVOPT_ID}" /><span class="lbl"></span></label>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${var.INSTOCK_NO}</td>
										<td>${var.BOX_NO}</td>
										<td>${var.PREFIX_PROD}</td>
										<td>${var.PROD_CODE}</td>
										<td>${var.PREFIX_QTY}</td>
										<td>${var.RECEIV_QTY}</td>
										<td>${var.COO}</td>
										<td>${var.PACKAGE_QTY}</td>
										<td>${var.BOARD_NO}</td>
										<td>${var.QR_CODE}</td>
										<td>${var.PREFIX_LOTNO}</td>
										<td>${var.LOT_NO}</td>
										<td>${var.PREFIX_DATECODE}</td>
										<td>${var.DATE_CODE}</td>
										<td>${var.PREFIX_SCAN}</td>
										<td>${var.SCAN_CODE}</td>
										<td>${var.SCAN_QTY}</td>
                                    ...
                                    <div class='hidden-phone visible-desktop btn-group'>
                                        ...
                                        <c:if test="${QX.edit != 1 && QX.del != 1 }">
                                            <span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="icon-lock" title="无权限"></i></span>
                                             ...
                                             <c:if test="${QX.edit == 1 }">
                                                    <a title="编辑" onclick="edit('${var.RECEIVOPT_ID}');" class='btn btn-mini btn-info' data-rel="tooltip" title="" data-placement="left"><i class="icon-edit"></i></a>
                                                </c:if>
                                                <c:if test="${QX.del == 1 }">
                                                    <a title="删除" onclick="del('${var.RECEIVOPT_ID}');" class='btn btn-mini btn-danger'  data-rel="tooltip" title="" data-placement="left"><i class="icon-trash"></i></a>
                                                </c:if>
                                    </div>
                                </td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>receivopt/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
