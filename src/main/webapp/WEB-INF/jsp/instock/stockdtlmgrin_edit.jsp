...
<script type="text/javascript">
	...
	*/
	...
	function getProdcuctList(customerId) {
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrin/getProductList.do',
	...
	function getLocator(){
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrin/getLocatorList.do',
	...
	function hadProd(PRODUCT_ID,STOCKMGRIN_ID) {
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>stockmgrin/hadProd.do',
	...
	function getPackRule() {
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrin/getPackRule.do',
	...
	function getProdProperty() {
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>stockmgrin/getProdProperty.do',
			...
			success: function (data) {
				...
				for (var k in data) {
					...
					for (var i = 0; i < lenth; i++) {
						...
						var UNIT_VOLUME = (0.01*LONG_UNIT*0.01*WIDE_UNIT*0.01*HIGH_UNIT).toFixed(4);
						//毛重、净重、单价
						var TOTAL_WEIGHT = json[i]['TOTAL_WEIGHT'];
						var TOTAL_SUTTLE = json[i]['TOTAL_SUTTLE'];
						var UNIT_PRICE = json[i]['UNIT_PRICE'];

						$("#TOTAL_WEIGHT_1").val(TOTAL_WEIGHT);
						$("#TOTAL_SUTTLE_1").val(TOTAL_SUTTLE);
						$("#UNIT_PRICE_1").val(UNIT_PRICE);
					...
					/*	$("#OT_EA").val(json[i]['OTHER_NUM']);
						...
						$("#EA_EA").val(EA_EA);*/
						$("#WEIGHT_UNIT_TYPE ").val(json[i]['TOTAL_WEIGHT_UNIT']);
						$("#SUTTLE_UNIT_TYPE ").val(json[i]['TOTAL_SUTTLE_UNIT']);
						$("#VOLUME_UNIT_TYPE ").val(json[i]['VOLUME_UNIT']);
...
<body>
	<form action="stockmgrin/${msg }.do" name="Form" id="Form" method="post">
		...
		<input type="hidden" name="TOTAL_SUTTLE_1" id="TOTAL_SUTTLE_1" value="${pd.TOTAL_SUTTLE_1}"/>
		<input type="hidden" name="UNIT_PRICE_1" id="UNIT_PRICE_1" value="${pd.UNIT_PRICE_1}"/>
										...
										<div class="span7">
											<table id="table_report0" class="table table-striped table-hover table-condensed">
												...
												<tr>
													...
													<td><input type="number" onkeyup="calcVolume();" min="0" name="LONG_UT" id="LONG_UT" value="${pd.LONG_UT}" maxlength="32" placeholder="0" title="长CM"/></td>
												</tr>
												...
												<tr>
													...
													<td><input type="number" min="0" onkeyup="calcVolume();" name="WIDE_UT" id="WIDE_UT" value="${pd.WIDE_UT}" maxlength="32" placeholder="0" title="宽CM"/></td>
												</tr>
												...
												<tr>
													...
													<td><input type="number"  min="0" onkeyup="calcVolume();" name="HIGH_UT" id="HIGH_UT" value="${pd.HIGH_UT}" maxlength="32" placeholder="0" title="高CM"/></td>
												</tr>
	   ...
	   <%-- <script src="static/1.9.1/jquery.min.js"></script>--%>
		...
		function calcWeight() {
			...
			var UNIT_WEIGHT = $("#TOTAL_WEIGHT_1").val();
			//EA IP CS PL OT
			//单位为其他时34 35 36 37 38
			var cnt = 0;
			if(unit_type =="24") {
				cnt = $("#OT_EA").val();
			}
		...
		function calcSuttle() {
			...
			var UNIT_SUTTLE = $("#TOTAL_SUTTLE_1").val();
			//EA IP CS PL OT
			//单位为其他时  30 31 32 33 34
			var cnt = 0;
			if(unit_type =="30") {
				cnt = $("#OT_EA").val();
			}
		...
		function calcUnitVolume() {
			...
			var VOLUME_CM = (0.01*LONG_UT * 0.01*WIDE_UT *0.01*HIGH_UT).toFixed(4);
			//var VOLUME_M = VOLUME_CM;
			$("#UNIT_VOLUME").val(VOLUME_CM);
		}
		...
		function calcTotalVolume() {
           ...
           /* alert(UNIT_VOLUME);
			...
			$("#TOTAL_VOLUME").val((cnt*UNIT_VOLUME).toFixed(4));
		}
