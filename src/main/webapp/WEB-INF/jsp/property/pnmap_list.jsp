...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="pnmap/list.do" method="post" name="Form" id="Form">
					...
					<div class="widget-body">
								...
								<div class="content">
									<table id="table_prod_dtl" class="table table-striped table-condensed table-hover">
										<tr>
											...
											<td style="width:70px;text-align: right;padding-top: 13px;">扫描料号</td>
											<td><input type="text" name="SCAN_PN" id="SCAN_PN" value="${pd.SCAN_PN}" maxlength="32" placeholder=" " title="扫描料号"/></td>

											<td style="width:70px;text-align: right;padding-top: 13px;">实际料号</td>
											<td><input type="text" name="ACTUAL_PN" id="ACTUAL_PN" value="${pd.ACTUAL_PN}" maxlength="32" placeholder=" " title="实际料号"/></td>
						...
						<div class="widget-toolbox padding-8 clearfix">
							<div class="btn-group">
								...
								<c:if test="${QX.edit == 1 }">
									<a class="btn btn-mini btn-warning" onclick="fromExcel();"  title="从EXCEL导入"><span class="icon-cloud-upload"></span>导入</a>
								</c:if>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr id="${var.PNMAP_ID}">
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${var.PNMAP_ID}" /><span class="lbl"></span></label>
								</td>
                                    ...
                                    <div class='hidden-phone visible-desktop btn-group'>
                                        ...
                                        <c:if test="${QX.edit != 1 && QX.del != 1 }">
                                            <span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="icon-lock" title="无权限"></i></span>
                                             ...
                                             <c:if test="${QX.edit == 1 }">
                                                    <a title="编辑" onclick="edit('${var.PNMAP_ID}');" class='btn btn-mini btn-info' data-rel="tooltip" title="" data-placement="left"><i class="icon-edit"></i></a>
                                                </c:if>
                                                <c:if test="${QX.del == 1 }">
                                                    <a title="删除" onclick="del('${var.PNMAP_ID}');" class='btn btn-mini btn-danger'  data-rel="tooltip" title="" data-placement="left"><i class="icon-trash"></i></a>
                                                </c:if>
                                    </div>
                                </td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>pnmap/goAdd.do';
			 diag.Width = 700;
			 diag.Height = 450;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>pnmap/delete.do?PNMAP_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>pnmap/goEdit.do?PNMAP_ID='+Id;
			 diag.Width = 700;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>pnmap/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>pnmap/excel.do';
		}
		...
		function fromExcel(){
			...
			diag.URL = '<%=basePath%>pnmap/goUploadExcel.do';
			diag.Width = 700;
			diag.Height = 450;
			diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
