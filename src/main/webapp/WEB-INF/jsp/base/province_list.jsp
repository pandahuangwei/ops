...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="province/list.do" method="post" name="Form" id="Form">
									...
									<table id="table_prod_dtl" class="table table-striped table-condensed table-hover">
										<tr>
											...
											<td style="vertical-align:top;">
												<select class="chzn-select" name="COUNTRY_ID" id="COUNTRY_ID" data-placeholder=" " style="vertical-align:top;" maxlength="32">
													<option value=""></option>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">省/州代码</th>
						<th class="center">省/州名称</th>
						<th class="center">国家</th>
						<th class="center">省/州英文名称</th>
				...
				<c:choose>
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr id="${var.PROVINCE_ID}">
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${var.PROVINCE_ID}" /><span class="lbl"></span></label>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${var.PROVINCE_CODE}</td>
										<td>${var.PROVINCE_CN}</td>
										<td>${var.COUNTRY_CN}</td>
										<td>${var.PROVINCE_EN}</td>
										<td>${var.MEMO}</td>
								...
								<%--<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="edit('${var.PROVINCE_ID}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:if test="${QX.del == 1 }">
											<li><a style="cursor:pointer;" title="删除" onclick="del('${var.PROVINCE_ID}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
											</c:if>
										</ul>
										</div>
									</div>
								...
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit == 1 }">
											<a title="编辑" onclick="edit('${var.PROVINCE_ID}');" class="btn btn-mini btn-info" data-rel="tooltip" title="" data-placement="left"><i class="icon-edit"></i></a>>
										</c:if>
										<c:if test="${QX.del == 1 }">
											<a title="删除" onclick="del('${var.PROVINCE_ID}');" class="btn btn-mini btn-danger" data-rel="tooltip" title="" data-placement="left"><i class="icon-trash"></i> </a>
										</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>province/goAdd.do?tm='+new Date().getTime();
			 diag.Width = 700;
			 diag.Height = 450;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>province/delete.do?PROVINCE_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>province/goEdit.do?PROVINCE_ID='+Id;
			 diag.Width = 700;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>province/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>province/excel.do';
		}
		</script>
