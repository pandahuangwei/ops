...
<body>
	<form action="locator/readExcel.do" name="Form" id="Form" method="post" enctype="multipart/form-data">
		...
		<div id="zhongxin">
					...
					<div class="widget-toolbox">
						<div class="btn-toolbar">
							<div class="btn-group">
								...
								<a class="btn btn-mini btn-success" onclick="window.location.href='<%=basePath%>/locator/downExcel.do'">下载模版</a>
							</div>
						</div>
					</div>
			...
			<c:choose>
				<c:when test="${not empty pd.IMP_SUCC}">
					...
					<table id="table_report_rs" class="table table-striped table-bordered table-hover">
						...
						<c:if test="${QX.cha == 1 }">
							<c:forEach items="${xlsList}" var="it" varStatus="vs">
								<td>${it.LOCATOR_CODE}</td>
								<td>${it.STORAGE_CODE}</td>
								<td>${it.MEMO}</td>
								</tr>
