...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="city/list.do" method="post" name="Form" id="Form">
									...
									<table id="table_prod_dtl" class="table table-striped table-condensed table-hover">
										<tr>
											...
											<td style="vertical-align:top;">
												<select class="chzn-select" name="COUNTRY_ID" id="COUNTRY_ID" value="${pd.COUNTRY_ID}" data-placeholder=" " style="vertical-align:top;" maxlength="32"
											...
											<td style="vertical-align:top;">
												<select   name="PROVINCE_ID" id="PROVINCE_ID" data-placeholder=" " value="${pd.PROVINCE_ID}"  style="vertical-align:top;" maxlength="32">
											...
											<td style="width:70px;text-align: right;padding-top: 13px;">城市代码</td>
											<td><input type="text" name="CITY_CODE" id="CITY_CODE" value="${pd.CITY_CODE}" maxlength="32" placeholder="" title="城市代码" onblur="hasCode()"/></td>

											<td style="width:70px;text-align: right;padding-top: 13px;">城市名称</td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">城市代码</th>
						<th class="center">上级城市代码</th>
						<th class="center">城市名称</th>
						<th class="center">是否报价线路</th>
						<th class="center">城市英文名称</th>
						<th class="center">城市区域</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr id="${var.CITY_ID}">
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${var.CITY_ID}" /><span class="lbl"></span></label>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${var.PROVINCE_CN}</td>
										<td>${var.CITY_CODE}</td>
										<td>${var.CITY_PARENT_CN}</td>
										<td>${var.CITY_CN}</td>
										<td>${var.OFFER_FLG}</td>
										<td>${var.CITY_EN}</td>
										<td>${var.CITY_AREA}</td>
								...
								<%--<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="edit('${var.CITY_ID}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:if test="${QX.del == 1 }">
											<li><a style="cursor:pointer;" title="删除" onclick="del('${var.CITY_ID}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
											</c:if>
										</ul>
										</div>
									</div>
									...
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit != 1 && QX.del != 1 }">
											<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="icon-lock" title="无权限"></i></span>
												...
												<c:if test="${QX.edit == 1 }">
													<a  title="编辑" onclick="edit('${var.CITY_ID}');" class='btn btn-mini btn-info' data-rel="tooltip" title="" data-placement="left"><i class="icon-edit"></i></a></li>
												</c:if>
												<c:if test="${QX.del == 1 }">
													<a  title="删除" onclick="del('${var.CITY_ID}');" class='btn btn-mini btn-danger' data-rel="tooltip" title="" data-placement="left"><i class="icon-trash"></i> </a></li>
												</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>city/goAdd.do';
			 diag.Width = 700;
			 diag.Height = 450;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>city/delete.do?CITY_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>city/goEdit.do?CITY_ID='+Id;
			 diag.Width = 700;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>city/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>city/excel.do';
		}
		</script>
