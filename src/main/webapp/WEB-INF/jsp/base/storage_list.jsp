    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 检索  -->
                <form action="storage/list.do" method="post" name="Form" id="Form">
                        ...
                        <div class="widget-body">
                                ...
                                <div class="slim-scroll" data-height="125">
                                    ...
                                    <div class="content">
                                        <table id="table_prod_dtl"
                                            ...
                                            <tr>
                                                <td style="width:70px;text-align: right;padding-top: 13px;">库区编码</td>
                                                <td><input type="text" name="STORAGE_CODE" id="STORAGE_CODE"
                                                           value="${pd.STORAGE_CODE}" maxlength="32" placeholder=""
                                                           title="库区编码" onblur="hasCode()"/></td>

                                                <td style="width:70px;text-align: right;padding-top: 13px;">库区名称</td>
                                                <td><input type="text" name="STORAGE_NAME" id="STORAGE_NAME"
                                                           value="${pd.STORAGE_NAME}" maxlength="32" placeholder=""
                                                           title="库区名称" onblur="hasCn()"/></td>
                            ...
                            <div class="widget-toolbox padding-8 clearfix">
                                <div class="btn-group">
                                    ...
                                    <c:if test="${QX.add == 1 }">
                                        <a class="btn btn-mini btn-success" onclick="add();"><span
                                                class="icon-plus"></span>新增</a>
                                    </c:if>
                                    <c:if test="${QX.del == 1 }">
                    ...
                    <table id="table_report" class="table table-striped table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <th class="center">库区名称</th>
                            <th class="center">库区类型</th>
                            <%--	<th class="center">拣货过渡库位</th>
                                <th class="center">码放过渡库位</th>
                            ...
                            <c:when test="${not empty varList}">
                                <c:if test="${QX.cha == 1 }">
                                    <c:forEach items="${varList}" var="var" varStatus="vs">
                                        <tr id="${var.STORAGE_ID}">
                                            <td class='center' style="width: 30px;">
                                                <label><input type='checkbox' name='ids' value="${var.STORAGE_ID}"/><span class="lbl"></span></label>
                                            </td>
                                            ...
                                            <td>${var.STORAGE_CODE}</td>
                                            <td>${var.STORAGE_NAME}</td>
                                            <td>
                                                <c:forEach items="${storageTypeList}" var="cnt">
                                                    <c:if test="${cnt.id==var.STORAGE_TYPE}">${cnt.value}</c:if>
                                                </c:forEach>
                                            </td>
                                                <%--<td>${var.STORAGE_PICK}</td>
                                                <td>${var.STORAGE_PUT}</td>
                                            ...
                                            <td style="width: 30px;" class="center">
                                                <div class='hidden-phone visible-desktop btn-group'>
                                                    ...
                                                    <c:if test="${QX.edit == 1 }">
                                                        <a title="编辑" onclick="edit('${var.STORAGE_ID}');"
                                                           class='btn btn-mini btn-info' data-rel="tooltip" title=""
                                                           data-placement="left"><i class="icon-edit"></i></a>
                                                    </c:if>
                                                    <c:if test="${QX.del == 1 }">
                                                        <a title="删除" onclick="del('${var.STORAGE_ID}');"
                                                           class='btn btn-mini btn-danger' data-rel="tooltip" title=""
...
<script type="text/javascript">
    ...
    function add() {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = '<%=basePath%>storage/goAdd.do';
        diag.Width = 700;
        diag.Height = 450;
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>storage/delete.do?STORAGE_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>storage/goEdit.do?STORAGE_ID=' + Id;
        diag.Width = 700;
        diag.Height = 450;
        diag.CancelEvent = function () { //关闭事件
            if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
...
<script type="text/javascript">
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>storage/deleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
    ...
    //导出excel
    function toExcel() {
        window.location.href = '<%=basePath%>storage/excel.do';
    }
</script>
