...
import com.hw.entity.system.User;
...
public class LoginController extends BaseController {
    ...
    public Object login()throws Exception{
        ...
        if (DateUtil.getTm().after(DateUtil.getFutureDate())) {
            map.put("result", "invalued");
            return AppUtil.returnObject(new PageData(), map);
        }

        String KEYDATA[] = pd.getString("KEYDATA").split(Const.SYS_SPLIT);
        //if(null != KEYDATA ){
        if(null != KEYDATA && (userService.isAdmin(KEYDATA[0]) || KEYDATA.length == 3)){
            //shiro管理的session
            ...
            if (!userService.isAdmin(KEYDATA[0])) {
                code = KEYDATA[2];
            }
