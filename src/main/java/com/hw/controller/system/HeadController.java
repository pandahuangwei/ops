...
public class HeadController extends BaseController {
    ...
    public ModelAndView goEditEmail() throws Exception{
        ...
        if(null != strEMAIL && !"".equals(strEMAIL)){
            String strEM[] = strEMAIL.split(Const.SYS_SPLIT);
            if(strEM.length == 4){
                pd.put("SMTP", strEM[0]);
                pd.put("PORT", strEM[1]);
        ...
        if(null != strSMS1 && !"".equals(strSMS1)){
            String strS1[] = strSMS1.split(Const.SYS_SPLIT);
            if(strS1.length == 2){
        ...
        if(null != strSMS2 && !"".equals(strSMS2)){
            String strS2[] = strSMS2.split(Const.SYS_SPLIT);
            if(strS2.length == 2){
        ...
        if(null != strFWATERM && !"".equals(strFWATERM)){
            String strFW[] = strFWATERM.split(Const.SYS_SPLIT);
            if(strFW.length == 5){
        ...
        if(null != strIWATERM && !"".equals(strIWATERM)){
            String strIW[] = strIWATERM.split(Const.SYS_SPLIT);
            if(strIW.length == 4){
        ...
        if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
            String strIW[] = strWEBSOCKET.split(Const.SYS_SPLIT);
            if(strIW.length == 4){
    ...
    public ModelAndView saveSys() throws Exception{
        ...
        Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+Const.SYS_SPLIT+pd.getString("PORT")+Const.SYS_SPLIT+pd.getString("EMAIL")+Const.SYS_SPLIT+pd.getString("PAW"));	//写入邮件服务器配置
        Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+Const.SYS_SPLIT+pd.getString("SMSPAW1"));	//写入短信1配置
        Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+Const.SYS_SPLIT+pd.getString("SMSPAW2"));	//写入短信2配置
        mv.addObject("msg","OK");
    ...
    public ModelAndView saveSys2() throws Exception{
        ...
        PageData pd = new PageData();
        pd = this.getPageData();
        Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+Const.SYS_SPLIT+pd.getString("fcontent")+Const.SYS_SPLIT+pd.getString("fontSize")+Const.SYS_SPLIT+pd.getString("fontX")+Const.SYS_SPLIT+pd.getString("fontY"));	//文字水印配置
        Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+Const.SYS_SPLIT+pd.getString("imgUrl")+Const.SYS_SPLIT+pd.getString("imgX")+Const.SYS_SPLIT+pd.getString("imgY"));	//图片水印配置
        Watermark.fushValue();
        mv.addObject("msg","OK");
        mv.setViewName("save_result");
        return mv;
    ...
    public ModelAndView saveSys3() throws Exception{
        ...
        Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+Const.SYS_SPLIT+pd.getString("WIMPORT")+Const.SYS_SPLIT+pd.getString("OLIP")+Const.SYS_SPLIT+pd.getString("OLPORT"));	//websocket配置
        mv.addObject("msg","OK");
        mv.setViewName("save_result");
        return mv;
    }
